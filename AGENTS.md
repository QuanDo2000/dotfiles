...
1. **config/shared/** - Cross-platform configs (neovim, starship, jj, SSH, AI tool seeds, Obsidian settings). Git settings for Linux/macOS are declared through Home Manager `programs.git`; `config/shared/.gitconfig` is kept for Windows.
