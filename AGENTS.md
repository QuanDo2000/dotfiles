...
1. **config/shared/** - Cross-platform configs (neovim, starship, jj, SSH, AI tool seeds, Obsidian settings). Raw Neovim config remains cross-platform, but the `fff.nvim` plugin/backend is Unix-only and disabled on Windows. Git settings for Linux/macOS are declared through Home Manager `programs.git`; `config/shared/.gitconfig` is kept for Windows.
