...
- **scripts/** — Modular bash scripts sourced by the unix `dotfile`:
  ...
  - `extras.sh` — zsh plugins (cloned to `~/.local/share/zsh/plugins`) and the directly-cloned tmux plugins (tmux-yank, catppuccin); no tmux plugin manager (sensible/pain-control are inlined in `.tmux.conf`). Also clones the ponytail OpenCode plugin to `~/.local/share/ponytail` (OpenCode has no marketplace for it, unlike Claude/Codex) and links its `/ponytail` commands into `~/.config/opencode/command/`; `opencode.json` references the plugin via `~/.local/share/ponytail/.opencode/plugins/ponytail.mjs`. The prompt is starship (config at `config/shared/starship/starship.toml`, symlinked to `~/.config/starship.toml` on all platforms).
