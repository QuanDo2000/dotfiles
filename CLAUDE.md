...
- **scripts/** — Modular bash scripts sourced by the unix `dotfile`:
  ...
  - `extras.sh` — zsh plugins (cloned to `~/.local/share/zsh/plugins`) and the directly-cloned tmux plugins (tmux-yank, catppuccin); no tmux plugin manager (sensible/pain-control are inlined in `.tmux.conf`). Also installs plugins for Codex (ponytail + superpowers) and OpenCode (ponytail). Codex: `codex plugin add` populates the machine-local plugin cache (`dotfiles.config.toml` only *declares* the plugins for `codex -p dotfiles`; without the cache, runtime activation silently no-ops). ponytail needs its marketplace added first; superpowers ships in the built-in `openai-curated` marketplace, but that marketplace only becomes available after `codex login` AND an interactive `/plugins` picker session populates `~/.codex/.tmp/plugins/` (a clone of github.com/openai/plugins). If `codex plugin add superpowers@openai-curated` fails for that reason, `install_codex_plugins` prints recovery steps instead of dying. OpenCode has no marketplace for ponytail, so the repo clones ponytail to `~/.local/share/ponytail` and links its `/ponytail` commands into `~/.config/opencode/command/`; `opencode.json` references the plugin via `~/.local/share/ponytail/.opencode/plugins/ponytail.mjs`. The prompt is starship (config at `config/shared/starship/starship.toml`, symlinked to `~/.config/starship.toml` on all platforms).
