...
- **scripts/** — Modular bash scripts sourced by the unix `dotfile`:
  ...
  - `extras.sh` — zsh plugins (cloned to `~/.local/share/zsh/plugins`) and the directly-cloned tmux plugins (tmux-yank, catppuccin); no tmux plugin manager (sensible/pain-control are inlined in `.tmux.conf`). Also installs the ponytail plugin for Codex and OpenCode. Codex: `codex plugin marketplace add` + `codex plugin add` populate the machine-local plugin cache (`dotfiles.config.toml` only *declares* the marketplace/plugin for `codex -p dotfiles`; without the cache, runtime activation silently no-ops). OpenCode has no marketplace for it, so the repo clones ponytail to `~/.local/share/ponytail` and links its `/ponytail` commands into `~/.config/opencode/command/`; `opencode.json` references the plugin via `~/.local/share/ponytail/.opencode/plugins/ponytail.mjs`. The prompt is starship (config at `config/shared/starship/starship.toml`, symlinked to `~/.config/starship.toml` on all platforms).
