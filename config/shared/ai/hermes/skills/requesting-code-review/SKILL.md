...
Run repository checks appropriate to the changed surface; inspect security and failure propagation.
...
Scan added lines in the requested diff only. Inspect matches and include confirmed concerns in Review and disposition.
...
Run the repository's configured tests, lint, and type checks on the requested revision.
