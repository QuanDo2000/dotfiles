...
# Install a stub `bun` on PATH that records every invocation's argv to
# $HOME/.bun_argv (one line per call) and exits 0. Stub `bun upgrade` too so
# the --update path doesn't try to network. Also scrubs PATH so the host
# machine's real `codex` / `bun` binaries don't bleed into `command -v`
# checks inside setup_codex. Used by the install/update tests below.
_install_bun_stub() {
...
# Regression: `bun update -g <pkg>` can silently no-op with "No package.json,
# so nothing to update" when the package isn't installed yet, so both branches
# must use `bun add -g` (idempotent install-or-upgrade).
test_setup_codex_install_uses_bun_add() {
  DRY=false
  _install_bun_stub

  setup_codex >/dev/null 2>&1

  local argv
  argv=$(cat "$HOME/.bun_argv" 2>/dev/null)
  assert_contains "$argv" "add -g @openai/codex"
  if [[ "$argv" == *"update -g"* ]]; then
    echo "  FAILED: install path must not use 'bun update -g'" >> "$ERROR_FILE"
  fi
  if [[ "$argv" == *"install -g"* ]]; then
    echo "  FAILED: install path must use 'bun add -g', not 'bun install -g'" >> "$ERROR_FILE"
  fi
}

test_setup_codex_update_uses_bun_add() {
  DRY=false
  _install_bun_stub
  # Pre-stage a `codex` so we exercise the existing-install --update branch.
  echo '#!/bin/bash' > "$HOME/.local/bin/codex"
  chmod +x "$HOME/.local/bin/codex"

  setup_codex --update >/dev/null 2>&1

  local argv
  argv=$(cat "$HOME/.bun_argv" 2>/dev/null)
  assert_contains "$argv" "add -g @openai/codex"
  if [[ "$argv" == *"update -g"* ]]; then
    echo "  FAILED: update path must not use 'bun update -g'" >> "$ERROR_FILE"
  fi
}

# `setup_codex --update` must propagate --update to `setup_bun` so that bun
# itself gets upgraded; the bun stub records the `upgrade` call.
test_setup_codex_update_propagates_to_bun() {
  DRY=false
  _install_bun_stub
  ...
  local argv
  argv=$(cat "$HOME/.bun_argv" 2>/dev/null)
  assert_contains "$argv" "upgrade"
}

# Inverse: without --update, setup_bun should NOT run `bun upgrade`.
test_setup_codex_install_does_not_upgrade_bun() {
  DRY=false
  _install_bun_stub
  ...
  local argv
  argv=$(cat "$HOME/.bun_argv" 2>/dev/null)
  if [[ "$argv" == *"upgrade"* ]]; then
    echo "  FAILED: install path must not run 'bun upgrade'" >> "$ERROR_FILE"
  fi
}
