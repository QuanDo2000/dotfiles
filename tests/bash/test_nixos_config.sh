...
test_home_config_puts_shared_user_tools_in_common_packages() {
  ...
  for pkg in neovim fd ripgrep gnupg nodejs ast-grep zig odin gleam erlang; do
...
test_home_config_manages_gitconfig_and_starship_config() {
  ...
  assert_contains "$home_text" "editor = \"nvim\""
...
test_home_config_owns_remaining_dotfiles() {
  ...
  assert_contains "$home_text" "forceSource ./mac/.zshrc.mac"
