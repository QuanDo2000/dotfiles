...
test_check_script_runs_repo_verification() {
  ...
  assert_contains "$check_text" 'nix build "$repo_dir#codex" "$repo_dir#obsidian-headless" "$repo_dir#pi-agent" "$repo_dir#fff-mcp" "$repo_dir#codebase-memory-mcp" --no-link'
